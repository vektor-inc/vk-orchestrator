...
| `links` | array | 外部リンク。`{ rel: "queue" \| "pr", url, label }`。実 http(s) URL のみ。無ければ空配列。 |
...
`option.command` は `commands.jsonl` の 1 行にそのまま書ける断片:

```json
{ "action": "set-status", "taskId": "139", "to": "awaiting-approval", "expected": "ready" }
```

- `action`: `"set-status"` \| `"set-priority"` \| `"set-sequential"`（新 action は作らない。既存契約のまま）。
- `taskId`: タスク ID（文字列）。
- `to`: 変更先の bare 値。
