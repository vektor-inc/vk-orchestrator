...
3. 完了コメントを投稿（オーケストレーターと同じ文言）。**URL の有無を問わず、既存の「✅ 完了」かつ「マージされました」コメントがあれば投稿しない**（下記スニペットは既存確認込み）

   ```bash
   BODIES=$(gh issue view <N> -R <task-queue> --json comments --jq '.comments[].body')
   if ! { printf '%s' "$BODIES" | grep -qF "✅ 完了" && printf '%s' "$BODIES" | grep -qF "マージされました"; }; then
