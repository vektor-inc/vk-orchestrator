...
本来はオーケストレーターが PR マージを検出してメタ issue をクローズするが、Claude がユーザー確認待ちで止まった際に `status:waiting-input` のまま放置される経路がある。task-queue 経由のタスクと認識したら、マージ後の cleanup で以下まで実施すること:

1. メタ issue 本文に PR URL を追記（オーケストレーターの標準形式）
