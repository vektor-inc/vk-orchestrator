...
test('resolveVkAgentsConfigPath: repoPath 指定時はリポジトリ直下 config.json を指す', () => {
  const repoPath = join(tmpdir(), 'vk-agents');
  assert.equal(
    resolveVkAgentsConfigPath({ vkAgents: { repoPath } }),
...
test('writeVkAgentsSettings: GUI の2キーだけを read-merge-write し、既存キーを保持する', () => {
...
test('writeVkAgentsSettings: config.json が無くても GUI 設定があれば新規作成する', () => {
  const dir = mkdtempSync(join(tmpdir(), 'vko-vkagents-'));
  try {
