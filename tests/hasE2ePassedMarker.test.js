/**
 * hasE2ePassedMarker のユニットテスト。
 *
 * automerge の e2e 完了ゲート（vektor-inc/vk-agents#136 / task-queue#167 案D）で使う判定。
 * マーカー = 'e2e-passed' ラベル + 「e2e-passed-sha: <sha>」コメント（現 head SHA と前方一致）。
...
describe('hasE2ePassedMarker', () => {
  it('(a) ラベルあり + SHA 一致コメントあり → true', async () => {
    const sha = 'abcdef1234567890abcdef1234567890abcdef12';
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: [`e2e の確認完了しました。\ne2e-passed-sha: ${sha}`],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, sha), true);
  });
  ...
  it('(b) ラベル無し → false（コメントが一致していても）', async () => {
    ...
    const client = makeClient({
      labels: [],
      comments: [`e2e-passed-sha: ${sha}`],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, sha), false);
  });
  ...
  it('(c) ラベルあり + SHA 不一致コメント → false', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: [`e2e-passed-sha: ${otherSha}`],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, headSha), false);
  });
  ...
  it('(e) 短縮 SHA の前方一致 → true', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: ['e2e-passed-sha: abcdef1'], // 7 桁短縮 SHA
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, headSha), true);
  });
  ...
  it('短縮 SHA でも別コミットには前方一致しない → false', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: ['e2e-passed-sha: abcde99'], // head とは異なる短縮 SHA
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, headSha), false);
  });
  ...
  it('大文字混じりの SHA コメントでも前方一致する（正規表現 /i・lowercase 正規化）', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: ['e2e-passed-sha: ABCDEF1'],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, headSha), true);
  });
  ...
  it('SHA は一致するが信頼境界外（author_association: NONE）の投稿者なら無視 → false（偽装防止）', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: [{ body: `e2e-passed-sha: ${sha}`, author_association: 'NONE' }],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, sha), false);
  });
  ...
  it('CONTRIBUTOR の SHA コメントは無視するが、信頼境界内の一致コメントが別にあれば true', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: [
        { body: `e2e-passed-sha: ${sha}`, author_association: 'CONTRIBUTOR' },
        { body: `e2e-passed-sha: ${sha}`, author_association: 'OWNER' },
      ],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, sha), true);
  });
  ...
  it('COLLABORATOR の一致コメントは信頼境界内として true', async () => {
    ...
    const client = makeClient({
      labels: ['e2e-passed'],
      comments: [{ body: `e2e-passed-sha: ${sha}`, author_association: 'COLLABORATOR' }],
    });
    assert.equal(await client.hasE2ePassedMarker(O, R, 10, sha), true);
  });
});
