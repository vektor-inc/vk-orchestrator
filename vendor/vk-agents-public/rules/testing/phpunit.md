...
`test_condition_name` キーには、どのような条件でどの結果を期待するテストなのかを日本語で記載してください。
テスト失敗時のメッセージとして出力されるため、読んですぐ状況が把握できる表現にしてください。

例）`'test_condition_name' => 'オプションが apple で個別指定が pen の場合 => pen'`

## WordPress の場合は WP_UnitTestCase を継承する

WordPress プロジェクトのテストクラスは `WP_UnitTestCase` を継承し、必要に応じて以下のメソッドを使用してください。
