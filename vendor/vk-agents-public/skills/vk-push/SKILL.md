...
- 認識するフラグは `--direct` **のみ**。`-` で始まるトークンのうち `--direct` 以外（例: `--force`, `-f`, `--no-verify`, `--amend` など）が含まれていた場合は、**git のオプションとして解釈せず**、エラーとして報告して中断する（`git push --force` のような危険なオプション混入を防ぐため）
...
1. `git status --porcelain` と `git diff --stat`、必要に応じて `git diff`（未ステージ）/ `git diff --cached`（ステージ済み）で変更内容を理解する
2. コミットメッセージを決める:
...
1. `git add -A` で全変更をステージする（特定ファイルのみの指示があれば従う）
2. ヒアドキュメントでコミットする:
   ```bash
